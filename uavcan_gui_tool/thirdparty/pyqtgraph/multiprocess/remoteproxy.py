        ...
        while True:
            try:
                ...
                if err.errno == 4:  ## interrupted system call; try again
                    ...
                    self.debugMsg('  handleRequest: got IOError %d from recv (%s); raise ClosedError.' % (
    ...
    def send(self, request, opts=None, reqId=None, callSync='sync', timeout=10, returnType=None, byteData=None, **kwds):
        ...
        **Arguments:**
        ...
        with self.sendLock:
            ...
            self.debugMsg(
                'send request: cmd=%s nByteMsgs=%d id=%s opts=%s' % (str(request[0]), nByteMsgs, str(reqId), str(opts)))
