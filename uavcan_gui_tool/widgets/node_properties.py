...
class InfoBox(QGroupBox):
    ...
    def _update(self):
        ...
        if entry.status:  # Status should be always available...
            ...
            self._mode_health_uptime[1].set(
                pyuavcan_v0.value_to_constant_name(entry.status, 'health', keep_literal=True))
...
class Controls(QGroupBox):
    def __init__(self, parent, node, target_node_id, file_server_widget, dynamic_node_id_allocator_widget):
        ...
        self._restart_button = make_icon_button('power-off', 'Restart the node [pyuavcan_v0.protocol.RestartNode]',
                                                self,
